					...
					<div class="card-header">
						<div class="box-tools pull-right">
							<div class="input-group input-group-sm" style="width: 300px;">
								<input type="text" v-model="posName" class="form-control" placeholder="职位名称">
						...
						<table class="table table-hover">
							<tbody>
								...
								<tr v-for="(item,index) in datas">
									<td>{{index+1}}</td>
									<td v-if="item.departmentModel != null">{{item.departmentModel.depName}}</td>
									<td v-else></td>
									<td>{{item.posName}}</td>
									<td>
...
<!-- position -->
<script>
	import bmadd from '@/components/zwgl/add.vue';
	import bmupdate from '@/components/zwgl/update.vue';
	...
	export default {
		...
		methods: {
			...
			getDatas: function() {
				...
				obj.$http({
					method: 'post',
					url: '/position/page/' + obj.$data.pageNow,
					data: {
